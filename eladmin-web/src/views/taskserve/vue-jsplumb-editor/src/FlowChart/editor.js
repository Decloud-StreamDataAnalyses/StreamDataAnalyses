 ...
 * @returns
 */
function addNodeByAction(action, elId, position, icon, value) {
  ...
  const targetEndpoints = [{ id: `target-${createUuid()}`, data: { value: '输入1' } },{ id: `target-${createUuid()}`, data: { value: '输入2' } }];
...
function addNodeByDrag(position, elId) {
  ...
  return addNodeByAction('drag', elId, position, icon, contentText);
}
...
function addNodeByCopy(position, nodeId) {
  ...
  return addNodeByAction('copy', elId, position, icon, value);
}
...
function execModel() {
  ...
  console.log('start to exece model....');
    // console.log('send to backup for java handing:', model.getXml()) ;//这里应该是一个回传到服务器的代码

  var xml = model.getXml().toString()
  console.log('execModel:')
  console.log(xml);
  model.sendString(xml);
...
function init() {
    ...
    bindEvent();
    API.getPropertyData().then((data) => {
        console.log('getPropertyData:', data.data, typeof(data.data))
        propertyDict = data.data;
        console.log('propertyDict:', propertyDict)
        for (var key in propertyDict) {
            console.log(key + "属性有:");
            var properties = propertyDict[key];
